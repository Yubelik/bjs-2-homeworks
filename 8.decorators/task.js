...
function cachingDecoratorNew(func) {
  ...
  function wrapper(...args) {
    // debu   gger;
        md5(args);
      const hash = md5.create(); // получаем правильный хеш c помощью функции md5
      let objectInCache = cache.find((item) => item == hash ); // ищем элемент, хеш которого равен нашему хешу
